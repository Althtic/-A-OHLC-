...
'''
Fama-French RMW 因子
对ROE与Size进行双重排序后求  1/2(S/R + B/R) - 1/2(S/W + B/W)
'''
# --- 配置日志 ---
...
def calculate_alpha(data) -> pd.DataFrame:
    ...
    data_eqy = pd.read_csv(r'C:\Users\63585\Desktop\PycharmProjects\pythonProject\QuantSystem\回测数据集\roe.csv',
                           usecols=['ts_code', 'trade_date', 'roe'],
                           low_memory=False
                           )  # total_hldr_eqy_exc_min_int 归母股东权益
    data_roe = data.merge(data_eqy, how='left', on=['ts_code', 'trade_date'])

    data_roe['rank_roe'] = data_roe.groupby('trade_date')['roe'].rank(pct=True)
    data_roe['rank_circ_mv'] = data_roe.groupby('trade_date')['circ_mv'].rank(pct=True)
    ...
    def spread_ret_calc(group, ret_col='dret'):
        ...
        if br.empty or sr.empty or bw.empty or sw.empty:
            return np.nan
        
        ret_robust = (br.mean() + sr.mean()) / 2
        ret_weak = (bw.mean() + sw.mean()) / 2
        return (ret_robust - ret_weak).round(7)
    daily_spread_series = data_roe.groupby('trade_date').apply(
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("正在保存数据...")
        save_data(processed_data, "rmw.csv")
        logger.info("数据已保存。")
    except Exception as e:
