...
def calculate_alpha(data) -> pd.DataFrame:
    ...
    try:
        ...
        def compute_corr(group):
            ...
            adv20_vals = group['adv20'].values
            n = len(adv20_vals)
            if n < 10: return pd.Series([np.nan] * n, index=group.index)
            corr_res = rolling_corr_numba(lows, adv20_vals, 5)
            return pd.Series(corr_res, index=group.index)
        data_c['adv20'] = data_c.groupby('ts_code')['vol'].rolling(window=20,min_periods=14).mean().reset_index(level=0, drop=True)
        data_c['corr_adv20_low_5'] = data_c.groupby('ts_code', group_keys=False).apply(compute_corr)
