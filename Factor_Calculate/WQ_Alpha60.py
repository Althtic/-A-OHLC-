...
def calculate_alpha(data) -> pd.DataFrame:
    ...
    # part 1 日内价格相对强度
    try:
        data_c['daily_relative_strength'] = ((data_c['close'] - data_c['low']) - (data_c['high'] - data_c['low'])) / (data_c['high'] - data_c['low'])
        data_c['vol_multi_rs'] = data_c['vol'] * data_c['daily_relative_strength']
        data_c['rank_vol_multi_rs'] = data_c.groupby('trade_date')['vol_multi_rs'].rank(pct=True)

        data_c['scale_rank_vol_multi_rs'] = data_c['rank_vol_multi_rs'].transform(
    ...
    try:
        ...
        ts_argmax_close_10 = calc_ts_rank(data_c['close'], codes, 10)
        data_c['ts_argmax_close_10'] = ts_argmax_close_10
        data_c['rank_ts_argmax_close_10'] = data_c.groupby('trade_date')['ts_argmax_close_10'].rank(pct=True)
        data_c['scale_rank_ts_argmax_close_10'] = data_c['rank_ts_argmax_close_10'].transform(
            lambda x: (x - x.mean()) / x.std() if x.std() > 0 else 0
        )
    except Exception as e:
    ...
    # part 3 计算差异组件（放大价格强度信号，并与近期价格峰值信号进行对比）与最终因子合成
          
    try:
        data_c['alpha_60'] = (2 * data_c['scale_rank_vol_multi_rs'] - data_c['scale_rank_ts_argmax_close_10'])
    ...
    return_columns = ['ts_code', 'trade_date', 'open', 'high', 'low', 'close', 'pre_close', 'industry_name', 'dret', 'raw_factor', factor_name]
...
if __name__ == "__main__":
    logger.info("--- 开始执行 Alpha#60 计算流程 ---")
    try:
        columns_needed = [
            'ts_code',
            'trade_date',
        ...
        logger.info("正在保存数据...")
        save_data(processed_data, "alpha_60.csv")
        logger.info("数据已保存。")
    except Exception as e:
