...
def price_back_adj(hist_data, adj_data):
    ...
    return_columns = ['ts_code', 'trade_date', 'adj_open', 'adj_high', 'adj_low', 'adj_close', 'adj_vol', 'adj_factor', 'amount', 'industry_name']
...
def remove_new_stock_initial_days(hist_data):
    ...
    # 标记是否为需要清洗的新股（首个交易日在20171010-20251231之间）
...
def calculate_dret(hist_data):
    hist_data['dret'] = hist_data.groupby('ts_code')['adj_close'].transform(
...
def is_trading_processing(hist_data, sus_data):
    ...
    is_one_word = (
            ...
            ((is_trade_processing_data['dret'] >= 0.09) | (is_trade_processing_data['dret'] <= -0.09)) &
            (is_trade_processing_data['adj_open'] == is_trade_processing_data['adj_close']) &
            (is_trade_processing_data['adj_high'] == is_trade_processing_data['adj_low']) &
            (is_trade_processing_data['adj_open'] == is_trade_processing_data['adj_high'])
    )
    conditions = [
        is_suspended,
        is_one_word
...
if __name__ == "__main__":
    ...
    # 保存结果
    pipeline.save('pipeline_test_data.csv')
